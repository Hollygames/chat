...
function init()
  ...
  chatWindow:getChildById('rB'):setMinimum(420)
	...
	consoleToggleChat = chatWindow:getChildById('toggleChat')
...
function Testador(text, speaktype, tabName, creatureName)
  local tab = getTab(tabName)
  if tab ~= nil then
    tester(text, speaktype, tab, creatureName)
  end
end
   ...
   if speaktype.npcChat and (g_game.getCharacterName() ~= creatureName or g_game.getCharacterName() == 'Account Manager') then
     ...
     if #highlightData > 0 then
       ...
       for i = 1, #highlightData / 3 do
         ...
         labelHighlight.onMousePress = function(widget, mousePos, button) sendMessage(dataBlock.words, consoleTabBar:getTab('NPCs')) end
         -- label.onClick = function(self) sendMessage(dataBlock.words, consoleTabBar:getTab('NPCs')) end
       ...
       elseif (#highlightOptions == 1) then
         labelHighlight.onMousePress = function(widget, mousePos, button) sendMessage(highlightOptions[1], consoleTabBar:getTab('NPCs')) end
       end
...
function tester(text, speaktype, tab, creatureName)
  if not tab or tab.locked or not text or #text == 0 then return end


  local panel = consoleTabBar:getTabPanel(tab)
  local consoleBuffer = panel:getChildById('consoleBuffer')
  local label = g_ui.createWidget('ItemLabel', consoleBuffer)
  label:setId('ItemLabel' .. consoleBuffer:getChildCount())

  label:setItemSubType(100)
  label:setItemId(3043)
  consoleTabBar:blinkTab(tab)
...
function onTalk(name, level, mode, message, channelId, creaturePos)
  ...
  elseif speaktype.private then
    ...
    if channel then
	  ...
	  if channel == "Loot-Channel" then
	  	print(""..composedMessage.."")
		Testador(composedMessage, speaktype, channel, name)
		end
    else
...
function toggleChat()
    ...
    buy_button = g_ui.createWidget("Button", chatWindow)
    buy_button:addAnchor(AnchorTop, "parent", AnchorTop)
    buy_button:addAnchor(AnchorLeft, "parent", AnchorLeft)
    buy_button:setMarginTop(228)
    buy_button:setMarginLeft(5)
    buy_button:setSize("30 23")
    buy_button:setIcon('/images/game/console/hide')
    ...
    buy_button:setVisible(true)
		...
		-- chatWindow:getChildById('hideChatButton'):setMarginRight(385)
